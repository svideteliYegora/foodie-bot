...
from models import con, User, Admin, Basket, Order, OrderComment, Good, GoodComment
...
# bot
bot = Bot(token=TG_TOKEN, parse_mode="types.ParseMode.MARKDOWN")
dp = Dispatcher()
...
async def handle_command_start(message: Message) -> None:
    ...
    if user_id in food_sdb.get_tg_ids():
        ...
        u_reg_questions[
            user_id] = 1  # ключ для получения вопроса из `INTRODUCTION_TEXT["user_introduction"]`
...
async def cb_handle_category(callback_query: CallbackQuery) -> None:
    ...
    # Получаем 5 последних отзыва к выбранному товару
...
async def cb_add_to_basket(callback_query: CallbackQuery) -> None:
    ...
    food_sdb.delete_all_record('basket', good_id=current_good['id'], user_id=u_id['id'])
    ...
    basket = food_sdb.add_record('basket',
                                 amount=f_cart["amount"] + total_amount,
                                 total_price=f_cart["amount"] * current_good["price"] + total_price,
                                 user_id=food_sdb.get_urecord_tg(user_id)["id"],
...
async def cb_basket_action(callback_query: CallbackQuery) -> None:
    ...
    if cb_data == "Оформить заказ":
        ...
        await callback_query.message.answer(text=text, reply_markup=ikb)
        ...
        await callback_query.bot.edit_message_text(chat_id=user_id, message_id=u_basket_dt["message_order_summary_id"],
                                                   text=BASKET_TEXT["empty_basket_message"])
